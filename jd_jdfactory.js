...
if ($.isNode()) {
  ...
  if (process.env.JDFACTORY_FORBID_ACCOUNT)
    process.env.JDFACTORY_FORBID_ACCOUNT.split('&').map((item, index) =>
      Number(item) === 0
        ? (cookiesArr = [])
          ...
          if (safeGet(data)) {
            ...
            if (data.data.bizCode === 0) {
              ...
              if (data.data.result.factoryInfo) {
                ...
                console.log(`\n已选商品：${name}`)
                console.log(
                  `当前已投入电量/所需电量：${useScore}/${totalScore}`
                )
                ...
                message += `当前完成度：${
                  ((remainScore * 1 + useScore * 1) / (totalScore * 1)).toFixed(
                    2
                  ) * 100
                }%\n`
                if (wantProduct) {
                  ...
                  for (let item of $.canMakeList) {
                    ...
                    if (
                      item.name.indexOf(wantProduct) > -1 &&
                      item.couponCount > 0
                    ) {
                  ...
                  if (
                    ...
                    console.log(
                      `\n提供的心仪商品${name}目前数量：${couponCount}，且当前总电量为：${
                        remainScore * 1 + useScore * 1
                      }，【满足】兑换此商品所需总电量：${totalScore + 100000}`
                    )
                    console.log(
                  ...
                  if (
                    ...
                    console.log(
                      `\n所选商品${name}目前数量：${couponCount}，且当前总电量为：${
                        remainScore * 1 + useScore * 1
                      }，【满足】兑换此商品所需总电量：${totalScore}`
                    )
                    console.log(
                      `BoxJs或环境变量暂未提供心仪商品，下面为您目前选的${name} 发送提示通知\n`
                    )
                ...
                if (wantProduct) {
                  ...
                  await jdfactory_getProductList(true)
                  message += `当前剩余最多商品：${
                    $.canMakeList[0] && $.canMakeList[0].name
                  }\n`
                  message += `兑换所需电量：${
                  ...
                  if (
                    $.canMakeList[0] &&
                    $.canMakeList[0].couponCount > 0 &&
                    $.batteryValue * 1 >= $.canMakeList[0] &&
                    $.canMakeList[0].fullScore
                  ) {
                    ...
                    if (new Date(nowTimes).getHours() === 12) {
                      $.msg(
                        ...
                        `京东账号${$.index}${
                          ...
                          $.canMakeList[0] && $.canMakeList[0] && [0].name
                        }所需总电量：${
                          $.canMakeList[0] && $.canMakeList[0].fullScore
                        }\n请点击弹窗直达活动页面\n选择此心仪商品并手动投入电量兑换`,
                        {
          ...
          if (err) {
            ...
            if (safeGet(data)) {
              ...
              if (data.data.bizCode === 0) {
                ...
                console.log(
                  `领取做完任务的奖励：${JSON.stringify(data.data.result)}`
                )
              } else {
...
function jdfactory_collectElectricity() {
  return new Promise((resolve) => {
    $.post(
      taskPostUrl('jdfactory_collectElectricity'),
      async (err, resp, data) => {
        try {
          if (err) {
            console.log(`${JSON.stringify(err)}`)
            ...
            if (safeGet(data)) {
              ...
              if (data.data.bizCode === 0) {
                console.log(
                  `成功收集${data.data.result.electricityValue}电量，当前蓄电池总电量：${data.data.result.batteryValue}\n`
                )
            ...
            if (safeGet(data)) {
              ...
              if (data.data.bizCode === 0) {
                ...
                $.taskVos.map((item) => {
                  if (item.taskType === 14) {
                    console.log(
                      `\n【京东账号${$.index}（${$.nickName || $.UserName}）的${
                        $.name
                      }好友互助码】${item.assistTaskDetailVo.taskToken}\n`
                    )
                  }
                })
              }
...
function jdfactory_makeProduct(skuId) {
  return new Promise((resolve) => {
    $.post(
      taskPostUrl('jdfactory_makeProduct', { skuId }),
      async (err, resp, data) => {
        try {
          if (err) {
            console.log(`${JSON.stringify(err)}`)
          ...
          if (safeGet(data)) {
            ...
            if (data.data.bizCode === 0) {
              ...
              if ($.newUser === 1) {
                //新用户
                console.log(
                  `此京东账号${$.index}${$.nickName}为新用户暂未开启${$.name}活动\n现在为您从库存里面现有数量中选择一商品`
                )
                if ($.haveProduct === 2) {
...
function readShareCode() {
  ...
  return new Promise(async (resolve) => {
    $.get(
      {
        url: `http://jd.turinglabs.net/api/v2/jd/ddfactory/read/${randomCount}/`,
        timeout: 10000,
      },
      (err, resp, data) => {
        try {
...
function requireConfig() {
  return new Promise((resolve) => {
    ...
    //Node.js用户请在jdCookie.js处填写京东ck;
    const shareCodes = $.isNode() ? require('./shareCode/jdFactoryShareCodes.js') : ''
...
function taskPostUrl(function_id, body = {}, function_id2) {
  ...
  return {
    ...
    headers: {
      ...
      Referer:
        'https://h5.m.jd.com/babelDiy/Zeus/2uSsV2wHEkySvompfjB43nuKkcHp/index.html',
      'User-Agent':
        'jdapp;iPhone;9.3.4;14.3;88732f840b77821b345bf07fd71f609e6ff12f43;network/4g;ADID/1C141FDD-C62F-425B-8033-9AAB7E4AE6A3;supportApplePay/0;hasUPPay/0;hasOCPay/0;model/iPhone11,8;addressid/2005183373;supportBestPay/0;appBuild/167502;jdSupportDarkMode/0;pv/414.19;apprpd/Babel_Native;ref/TTTChannelViewContoller;psq/5;ads/;psn/88732f840b77821b345bf07fd71f609e6ff12f43|1701;jdv/0|iosapp|t_335139774|appshare|CopyURL|1610885480412|1610885486;adk/;app_device/IOS;pap/JA2015_311210|9.3.4|IOS 14.3;Mozilla/5.0 (iPhone; CPU iPhone OS 14_3 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Mobile/15E148;supportJDSHWK/1',
    },
    timeout: 10000,
