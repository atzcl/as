...
async function jdCash() {
  ...
  await getReward('2')
  $.exchangeBeanNum = 0
  cash_exchange = $.isNode()
    ? process.env.CASH_EXCHANGE
      ? process.env.CASH_EXCHANGE
      : `${cash_exchange}`
    : $.getdata('cash_exchange')
    ? $.getdata('cash_exchange')
    : `${cash_exchange}`
...
function helpFriend(helpInfo) {
  return new Promise((resolve) => {
    $.get(
      taskUrl('cash_mob_assist', { ...helpInfo, source: 1 }),
      (err, resp, data) => {
        try {
          if (err) {
            console.log(`${JSON.stringify(err)}`)
...
function exchange2(node) {
  ...
  if (data['node'] === '-1') {
    body = `body=${encodeURIComponent(
      JSON.stringify(data)
    )}&uuid=8888888&client=apple&clientVersion=9.4.1&st=1619595890027&sign=92a8abba7b6846f274ac9803aa5a283d&sv=102`
  } else if (data['node'] === '0') {
    body = `body=${encodeURIComponent(
      JSON.stringify(data)
    )}&uuid=8888888&client=apple&clientVersion=9.4.1&st=1619597882090&sign=e00bd6c3af2a53820825b94f7a648551&sv=100`
  } else if (data['node'] === '1') {
    body = `body=${encodeURIComponent(
      JSON.stringify(data)
    )}&uuid=8888888&client=apple&clientVersion=9.4.1&st=1619595655007&sign=2e72bbd21e5f5775fe920eac129f89a2&sv=111`
  } else if (data['node'] === '2') {
    body = `body=${encodeURIComponent(
      JSON.stringify(data)
    )}&uuid=8888888&client=apple&clientVersion=9.4.1&st=1619597924095&sign=c04c70370ff68d71890de08a18cac981&sv=112`
  } else if (data['node'] === '3') {
    body = `body=${encodeURIComponent(
      JSON.stringify(data)
    )}&uuid=8888888&client=apple&clientVersion=9.4.1&st=1619597953001&sign=4c36b3d816d4f0646b5c34e7596502f8&sv=122`
  }
  return new Promise((resolve) => {
    const options = {
          ...
          if (safeGet(data)) {
            ...
            if (data['code'] === 0) {
              if (data.data.bizCode === 0) {
                console.log(
                  `花费${data.data.result.needMoney}元红包兑换成功！获得${data.data.result.beanName}\n`
                )
...
function readShareCode() {
  ...
  return new Promise(async (resolve) => {
    $.get(
      {
        url: `https://code.chiang.fun/api/v1/jd/jdcash/read/1/`,
        timeout: 10000,
      },
      (err, resp, data) => {
        try {
...
function shareCodesFormat() {
  return new Promise(async (resolve) => {
    ...
    if ($.shareCodesArr[$.index - 1]) {
      ...
      $.newShareCodes = [
        ...authorCode.map(
          (item, index) => (authorCode[index] = item['inviteCode'])
        ),
        ...$.newShareCodes,
      ]
    }
...
function getAuthorShareCode2(
  url = 'https://ghproxy.com/https://raw.githubusercontent.com/asd920/updateTeam/master/shareCodes/jd_updateCash.json'
) {
  return new Promise((resolve) => {
    $.get(
      {
