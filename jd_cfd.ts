...
let cookie: string = '',
  ...
  res: any = '',
  shareCodes: string[] = [
    '54F20ECB6680C7BD4657EA106D62D508CD79A71C733CF3362778B2086817052E',
    '5BE4E073D88BF3755AE0E7321538F34983462CC13A763310595646ED5BCAA087',
  ]
...
!(async () => {
  ...
  for (let i = 0; i < cookiesArr.length; i++) {
    ...
    res = await api(
      'user/QueryUserInfo',
      '_cfd_t,bizCode,ddwTaskId,dwEnv,ptag,source,strShareId,strZone',
    ...
    if (res.StoryInfo.StoryList) {
      ...
      if (res.StoryInfo.StoryList[0].Special) {
        console.log(
          `船来了，乘客是${res.StoryInfo.StoryList[0].Special.strName}`
        )
        let shipRes: any = await api(
        ...
        if (shipRes.iRet === 0) console.log('船客接待成功')
        else console.log('船客接待失败', shipRes)
      }
    ...
    for (let n = 0; n < bags.length; n++) {
      if (n % 2 === 0) strTypeCnt += `${bags[n]}:`
      else strTypeCnt += `${bags[n]}|`
    }
    if (bags.length !== 0) {
      res = await api(
        'story/sellgoods',
    ...
    if (!res.TourGuideList) {
      ...
      for (let e of res.TourGuideList) {
        if (e.strBuildIndex !== 'food' && e.ddwRemainTm === 0) {
          let employ: any = await api(
            'user/EmployTourGuide',
            '_cfd_t,bizCode,ddwConsumeCoin,dwEnv,dwIsFree,ptag,source,strBuildIndex,strZone',
            {
              ddwConsumeCoin: e.ddwCostCoin,
              dwIsFree: 0,
              strBuildIndex: e.strBuildIndex,
            }
          )
          if (employ.iRet === 0) console.log(`雇佣${e.strBuildIndex}导游成功`)
          if (employ.iRet === 2003) break
          await wait(1000)
        }
      }
    }
    ...
    for (let b of ['food', 'fun', 'shop', 'sea']) {
      ...
      if (res.dwCanLvlUp === 1) {
        res = await api(
          ...
          { ddwCostCoin: res.ddwNextLvlCostCoin, strBuildIndex: b }
        )
        if (res.iRet === 0) {
          console.log(`升级成功`)
          await wait(2000)
  ...
  for (let i = 0; i < cookiesArr.length; i++) {
    for (let j = 0; j < shareCodes.length; j++) {
      ...
      if (
        res.iRet === 2232 ||
        res.sErrMsg === '今日助力次数达到上限，明天再来帮忙吧~'
      ) {
        break
      }
      await wait(3000)
...
function api(fn: string, stk: string, params: Params = {}) {
  return new Promise(async (resolve) => {
    let url = `https://m.jingxi.com/jxbfd/${fn}?strZone=jxbfd&bizCode=jxbfd&source=jxbfd&dwEnv=7&_cfd_t=${Date.now()}&ptag=&_ste=1&_=${Date.now()}&sceneval=2&_stk=${encodeURIComponent(
      stk
    )}`
    if (['GetUserTaskStatusList', 'Award', 'DoTask'].includes(fn)) {
      console.log('api2')
      url = `https://m.jingxi.com/newtasksys/newtasksys_front/${fn}?strZone=jxbfd&bizCode=jxbfddch&source=jxbfd&dwEnv=7&_cfd_t=${Date.now()}&ptag=&_stk=${encodeURIComponent(
        stk
    ...
    if (Object.keys(params).length !== 0) {
      ...
      for (key in params) {
        if (params.hasOwnProperty(key)) url += `&${key}=${params[key]}`
      }
    }
...
function mainTask(fn: string, stk: string, params: Params = {}) {
  return new Promise(async (resolve) => {
    let url = `https://m.jingxi.com/newtasksys/newtasksys_front/${fn}?strZone=jxbfd&bizCode=jxbfd&source=jxbfd&dwEnv=7&_cfd_t=${Date.now()}&ptag=&_stk=${encodeURIComponent(
      stk
    )}&_ste=1&_=${Date.now()}&sceneval=2`
    if (Object.keys(params).length !== 0) {
      let key: keyof Params
...
function makeShareCodes() {
  return new Promise<void>(async (resolve, reject) => {
    ...
    axios
      .get(
        `https://api.sharecode.ga/api/autoInsert?db=jxcfd&code=${res.strMyShareId}&bean=${bean}&farm=${farm}&pin=${pin}`
      )
      .then((res) => {
        if (res.data.code === 200) console.log('已自动提交助力码')
        else console.log('提交失败！已提交farm和bean的cookie才可提交cfd')
        resolve()
      })
      .catch((e) => {
        console.log(e)
