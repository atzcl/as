...
class PuzzleRecognizer {
  ...
  recognize() {
    ...
    const posY =
      this.y + PUZZLE_PAD + (patchHeight - PUZZLE_PAD) / 2 - PUZZLE_GAP / 2
...
function getCoordinate(c) {
  ...
  for (var e = 0; e < c.length; e++) {
    if (e == 0) {
      ...
      b.push(
        pretreatment(c[e][2] < 4398046511103 ? c[e][2] : 4398046511103, 7, true)
      )
    } else {
      var a = c[e][0] - c[e - 1][0]
      var f = c[e][1] - c[e - 1][1]
...
class MousePosFaker {
  ...
  run() {
    ...
    const firstPos = [
      this.x - parseInt(Math.random() * 6, 10),
      this.y + parseInt(Math.random() * 11, 10),
      this.t,
    ]
    ...
    $.post(
      {
        ...
        headers: {
          ...
          accept: '*/*',
          'content-type':
            fnId === 'followGood' || fnId === 'followShop'
  ...
  return new Promise((resolve) => {
    $.post(
      ...
      (err, resp, data) => {
        data = JSON.parse(data)
        if (
          new Date().getTime() - new Date(data.data.lastFeedTime) <
          10800000
        ) {
          console.log('喂食间隔不够。')
          resolve()
    ...
    $.get(
      ...
      (err, resp, data) => {
        try {
          ...
          data = JSON.parse(data)
          data.errorCode === 'received'
            ? console.log(`\t任务成功！获得${data.data}狗粮`)
            : console.log('\t任务失败', JSON.stringify(data))
        } catch (e) {
          $.logErr(e)
        } finally {
    ...
    $.post(
      {
        ...
        headers: {
          ...
          'X-Requested-With': 'com.jingdong.app.mall',
          Referer:
            'https://h5.m.jd.com/babelDiy/Zeus/2wuqXrZrhygTQzYA7VufBEpj4amH/index.html?babelChannel=ttt12&sid=445902658831621c5acf782ec27ce21w&un_area=12_904_3373_62101',
          Origin: 'https://h5.m.jd.com',
          'User-Agent': $.isNode()
            ? process.env.JD_USER_AGENT
              ? process.env.JD_USER_AGENT
          ...
          async (err, resp, data) => {
            ...
            for (let f of data.datas) {
              if (f.stealStatus === 'can_steal') {
                ...
                $.get(
                  {
                    url: `https://jdjoy.jd.com/common/pet/enterFriendRoom?reqSource=h5&invokeKey=qRKHmL4sna8ZOP9F&friendPin=${encodeURIComponent(
                      f.friendPin
                    )}`,
                    headers: {
                      Host: 'jdjoy.jd.com',
                  ...
                  (err, resp, data) => {
                    $.get(
                      {
                        url: `https://jdjoy.jd.com/common/pet/getRandomFood?reqSource=h5&invokeKey=qRKHmL4sna8ZOP9F&friendPin=${encodeURIComponent(
                          f.friendPin
                        )}`,
                        headers: {
                          Host: 'jdjoy.jd.com',
