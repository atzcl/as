...
let cookie: string = '',
  ...
  data: any,
  UserName: string
let shareCodeSelf: string[] = [],
  shareCodePool: string[] = [],
  shareCode: string[] = [],
  shareCodeFile: object = require('./jdFruitShareCodes')
let message: string = '',
  h5stTool: H5ST = new H5ST('0c010', USER_AGENT, '8389547038003203')
...
!(async () => {
  ...
  for (let [index, value] of cookiesArr.entries()) {
    ...
    try {
        ...
        for (let i = res.waterFriendTaskInit.waterFriendCountKey; i < res.waterFriendTaskInit.waterFriendMax; i++) {
          for (let fr of friendList) {
            if (fr.friendState === 1) {
              ...
              if (data.code === '0') console.log(`给好友${fr.nickName}浇水成功`)
              if (data.cardInfo) {
                console.log('获得卡片')
              }
              await wait(2000)
      ...
      for (let code of shareCodeSelf) {
        ...
        data = await api('initForFarm', {
          shareCode: `${code}-3`,
          lng: '0.000000',
          lat: '0.000000',
          sid: '2871ac0252645ef0e2731aa7d03c1d3w',
          un_area: '16_1341_1347_44750',
          version: 14,
          channel: 1,
          babelChannel: 0,
        })
  ...
  if (message) await sendNotify('东东农场', message)
})()
